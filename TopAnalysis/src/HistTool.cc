  ...
  if (hist -> InheritsFrom("TH2")) 
    {
      ...
      all2dPlots_[title] -> SetDirectory(nullptr);
    }
  else 
    {
      allPlots_[title] = hist;
      allPlots_[title] -> SetDirectory(nullptr);
      if (nsyst_ > 0) 
	{
	  if ( all2dPlots_.find(title + "_syst") == all2dPlots_.end() ) 
	    {
	      // not found
	      ...
	      all2dPlots_[title + "_syst"] -> SetDirectory(nullptr);
	    } 
	  else 
	    {
	      // found
...
void HistTool::fill(TString title, double value, std::vector<double> weights, TString cat) 
{
  ...
  if (cat != "")
      ...
      if(not allPlots_.count(newTitle)) 
	{
	  ...
	  allPlots_[newTitle] -> SetDirectory(nullptr);
...
void HistTool::fill2D(TString title, double valueX, double valueY, std::vector<double> weights, TString cat) 
{
  ...
  if (cat != "")
      ...
      if(not all2dPlots_.count(newTitle)) 
	{
	  ...
	  all2dPlots_[newTitle] -> SetDirectory(nullptr);
