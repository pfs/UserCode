  ...
  applyFilt_                   ( iConfig.getParameter<bool>("applyFilt") )
{
  ...
  for(size_t ind = 0; ind < triggersToUse_.size(); ind ++) 
    histContainer_["triggerList"] -> GetXaxis() -> SetBinLabel(ind + 1, triggersToUse_[ind].c_str());
  histContainer_["counter"]         = fs->make<TH1F>("counter", "; Counter; Events", 2, 0.0, 2.0);
  histContainer_["fidcounter"]      = (TH1 *) fs -> make<TH2F>("fidcounter",    ";Variation;Events", 1500, 0.0, 1500.0, 11, 0, 11); 
  histContainer_["pu"]              = fs -> make<TH1F>("pu",      ";Pileup observed;Events / 1", 100, 0.0, 100.0);
  histContainer_["putrue"]          = fs -> make<TH1F>("putrue",  "; Pileup true;Events / 0.1", 100, 0.0, 100.0);
  for(std::unordered_map<std::string, TH1*>::iterator it = histContainer_.begin();   it != histContainer_.end();   it++ ) 
    it -> second -> Sumw2();
...
void MiniAnalyzer_test::genAnalysis(const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  for (ipu = PupInfo -> begin(); ipu != PupInfo -> end(); ++ ipu) 
	...
	continue; // storing detailed PU info only for BX=0
  ...
  if (evet.isValid())
    {
      const double asdd = evet -> originalXWGTUP();
      for(unsigned int ind = 0; ind < evet -> weights().size(); ind++)
	{
	  const double asdde = evet -> weights()[ind].wgt;
  ...
  if (genEventInfoProduct.isValid())
    {    
      for (unsigned int ind = 0; ind < genEventInfoProduct -> weights().size(); ind++)
	{
	  // ev_.g_psw[ev_.g_npsw] = genEventInfoProduct->weights().at(ind);
	  // ev_.g_npsw++;
	}
    }
  ...
  if(dressedLeptons.isValid()) 
      ...
      for(auto genLep = dressedLeptons -> begin();  genLep != dressedLeptons -> end(); ++ genLep)
	{
	  ...
	  //gen level selection
	  if (genLep -> pt() > 20.0 && fabs(genLep -> eta()) < 2.5) 
	    ngleptons_++;
	}
    }
  ...
  if (genPhotons.isValid())
      ...
      for (auto genPhoton = genPhotons -> begin();  genPhoton != genPhotons -> end(); ++ genPhoton)
	{
	  ...
	  //gen level selection
	  if(genPhoton -> pt() > 20.0 && fabs(genPhoton -> eta()) < 2.5) 
	    ngphotons_++;
	}
    }
  ...
  LorentzVector pvP4(0.0, 0.0, 0.0, 0.0);
  if (genParticles.isValid())
    {
      for (size_t ind = 0; ind < genParticles -> size(); ++ ind)
	{
	  const pat::PackedGenParticle & genIt = (*genParticles)[ind];
  ...
  if (prunedGenParticles.isValid())
    {
      for (size_t ind = 0; ind < prunedGenParticles -> size(); ++ ind)
	{
	  const reco::GenParticle & genIt = (*prunedGenParticles)[ind];
	  const int absid = abs(genIt.pdgId());
	  const bool outGoingProton( absid == 2212 && genIt.status() == 1 && fabs(genIt.eta()) > 4.7 );
	  const bool topLastCopy(absid == 6 && genIt.isLastCopy());
	  const bool wLastCopy(absid == 24 && genIt.isLastCopy());
	  const bool isHardProcess(genIt.isHardProcess() || genIt.statusFlags().fromHardProcess()|| genIt.statusFlags().fromHardProcessBeforeFSR());
  ...
  */
  ...
  if(genMet.isValid())
    {
      ev_.gtop_id [ ev_.ngtop ]  = 0.0;
      ev_.gtop_pt [ ev_.ngtop ]  = (*genMet)[0].pt();
      ev_.gtop_eta[ ev_.ngtop ]  = 0.0;
      ev_.gtop_phi[ ev_.ngtop ]  = (*genMet)[0].phi();
      ev_.gtop_m  [ ev_.ngtop ]  = 0.0;
      ev_.ngtop ++;
    }
  ...
  for(Int_t iw = 0; iw < ev_.g_nw; iw ++)
    {
      const Double_t x(iw);
      const Double_t wgt(ev_.g_w[iw]);
      TH2F * fidCounter = (TH2F *) histContainer_["fidcounter"];
      fidCounter -> Fill(x, 0.0, wgt);
      if (ngleptons_ > 0)                 fidCounter -> Fill(x, 1.0, wgt);
      if (ngleptons_ > 1)                 fidCounter -> Fill(x, 2.0, wgt);
      if (ngleptons_ > 0 && ngjets > 0)   fidCounter -> Fill(x, 3.0, wgt);
      if (ngleptons_ > 1 && ngjets > 0)   fidCounter -> Fill(x, 4.0, wgt);
      if (ngleptons_ > 0 && ngjets > 1)   fidCounter -> Fill(x, 5.0, wgt);
      if (ngleptons_ > 1 && ngjets > 1)   fidCounter -> Fill(x, 6.0, wgt);
      if (ngleptons_ > 0 && ngjets > 2)   fidCounter -> Fill(x, 7.0, wgt);
      if (ngleptons_ > 1 && ngjets > 2)   fidCounter -> Fill(x, 8.0, wgt);
      if (ngleptons_ > 0 && ngjets > 3)   fidCounter -> Fill(x, 9.0, wgt);
      if (ngleptons_ > 1 && ngjets > 3)   fidCounter -> Fill(x, 10.0,wgt);
    }
...
void MiniAnalyzer_test::recAnalysis(const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  for (unsigned int ind = 0; ind < h_trigRes -> size(); ind ++) 
    {	
      if( !(*h_trigRes)[ind].accept() ) 
	continue;
      for(size_t itrig = 0; itrig < triggersToUse_.size(); itrig ++)
	{
	  if (triggerList[ind].find(triggersToUse_[itrig]) == string::npos) 
	    continue;          
          const int prescale = h_trigPrescale -> getPrescaleForIndex(ind); 
          const int l1prescale = h_l1trigPrescale -> getPrescaleForIndex(ind); 
          if (itrig < 32)
	    {
	  ...
	  // histContainer_["triggerList"]     -> Fill(itrig);
          ...
          bool isZeroBias(triggerList[ind].find("ZeroBias") != string::npos);
          if(!isZeroBias) 
	    continue;
  ...
  */
  ...
  for (const pat::Muon & mu : *muons) 
    { 
      ...
      //apply correction
      float pt  = mu.pt();
      if (pt < 2.0) continue; //no need to care about very low pt muons here... (corrections will tend to be meaningless)
      float eta = mu.eta();
      float phi = mu.phi();
      float q   = mu.charge();
      ...
      if (iEvent.isRealData())
        {
          ...
          for (unsigned int ind = 0; ind < 100; ind ++)
            muStatUncReplicas[ind] = muonRC_ -> kScaleDT(q, pt, eta, phi, 1, ind);
      ...
      else
        {
          ...
          for (int ind = 0; ind < 100; ind ++)
            muStatUncReplicas[ind] = gen ?
              muonRC_ -> kSpreadMC(q, pt, eta, phi, gen -> pt(), 1, ind) :
              muonRC_ -> kSmearMC(q, pt, eta, phi, tlwm, smearSeed, 1, ind);
          statUnc = (1.0 + TMath::StdDev(muStatUncReplicas.begin(), muStatUncReplicas.end()))/sf;
          zptUnc = (gen ?
      ...
      //kinematics
      const bool passPt(p4.Pt() > 10.0);
      const bool passEta(fabs(p4.Eta()) < 2.5);
      if (!passPt || !passEta) 
	continue;
	...
	{
	  if (abs(ev_.g_id[ig]) != ev_.l_id[ev_.nl]) 
	    continue;
	  if (deltaR( mu.eta(), mu.phi(), ev_.g_eta[ig], ev_.g_phi[ig]) > 0.4) 
	    continue;
	  ev_.l_g[ev_.nl] = ig;
	  break;
	}	 
      ...
      ev_.l_miniIso[ev_.nl]          = getMiniIsolation(pfcands, &mu, 0.05, 0.2, 10.0, false);
      ev_.l_relIso[ev_.nl]           = ( 
					mu.pfIsolationR04().sumChargedHadronPt 
					+ max(0.0, mu.pfIsolationR04().sumNeutralHadronEt + mu.pfIsolationR04().sumPhotonEt - 0.5*mu.pfIsolationR04().sumPUPt)
					 ) / p4.Pt();
      ev_.l_ip3d[ev_.nl]             = -9999.0;
      ...
      if (mu.innerTrack().get())
	{
      ...
      if( p4.Pt() > 20.0 && fabs(p4.Eta()) < 2.5 && isLoose) 
	nrecleptons_++;
    }
  ...
  for (const pat::Electron & e : * electrons) 
    {        
      ...
      //kinematics cuts
      const bool passPt(corrP4.pt() > 15.0);
      const bool passEta(fabs(corrP4.eta()) < 2.5 && (fabs(e.superCluster() -> eta()) < 1.4442 || fabs(e.superCluster() -> eta()) > 1.5660));
      if (!passPt || !passEta) 
	continue;
      
      //full id+iso decisions
      const bool isVeto           ( e.electronID("cutBasedElectronID-Fall17-94X-V1-veto") );
      const int  vetoBits         ( e.userInt("cutBasedElectronID-Fall17-94X-V1-veto")  );
      const bool passVetoId       ( (vetoBits | 0xc0) == 0x3ff);  //mask isolation cuts and require all bits active      
      const bool isLoose          ( e.electronID("cutBasedElectronID-Fall17-94X-V1-loose") );
      const int  looseBits        ( e.userInt("cutBasedElectronID-Fall17-94X-V1-loose")  );
      const bool passLooseId      ( (looseBits | 0xc0) == 0x3ff);  //mask isolation cuts and require all bits active
      const bool isMedium         ( e.electronID("cutBasedElectronID-Fall17-94X-V1-medium") );
      const int  mediumBits       ( e.userInt("cutBasedElectronID-Fall17-94X-V1-medium")  );
      const bool passMediumId     ( (mediumBits | 0xc0) == 0x3ff);  //mask isolation cuts and require all bits active
      const bool isTight          ( e.electronID("cutBasedElectronID-Fall17-94X-V1-tight") );
      const int  tightBits        ( e.userInt("cutBasedElectronID-Fall17-94X-V1-tight") );
      const bool passTightId      ( (tightBits | 0xc0) == 0x3ff);  //mask isolation cuts and require all bits active

      const bool mvawp80          (e.electronID("mvaEleID-Fall17-iso-V2-wp80"));
      const bool mvawp90          (e.electronID("mvaEleID-Fall17-iso-V2-wp90"));
      const bool mvawploose       (e.electronID("mvaEleID-Fall17-iso-V2-wpLoose"));
      const bool mvanonisowp80    (e.electronID("mvaEleID-Fall17-noIso-V2-wp80"));
      const bool mvanonisowp90    (e.electronID("mvaEleID-Fall17-noIso-V2-wp90"));
      const bool mvanonisowploose (e.electronID("mvaEleID-Fall17-noIso-V2-wpLoose"));
      const bool passHEEP         (e.electronID("heepElectronID-HEEPV70"));
	...
	{
	  const float dxy(fabs(e.gsfTrack() -> dxy(primVtx.position())));
	  const float dz(fabs(e.gsfTrack() -> dz(primVtx.position())));
      ...
      for (int ig = 0; ig < ev_.ng; ig ++)
	{
	  ...
	  if (deltaR( corrP4.eta(), corrP4.phi(), ev_.g_eta[ig], ev_.g_phi[ig]) > 0.4) 
	    continue;
	  ev_.l_g[ev_.nl] = ig;
	  break;
	}	      
      ...
      ev_.l_miniIso[ev_.nl]          = getMiniIsolation(pfcands, & e, 0.05, 0.2, 10.0, false);
      ev_.l_relIso[ev_.nl]           = (e.chargedHadronIso() + max(0.0, e.neutralHadronIso() + e.photonIso()  - 0.5 * e.puChargedHadronIso()))/corrP4.pt();     
      ...
      if (e.gsfTrack().get())
	{
      ...
      if ( corrP4.pt()>20.0 && passEta && passLooseId ) 
	nrecleptons_++;
    }
  ...
  for (const pat::Photon & g : *photons)
	...
	continue;
      ...
      const int looseBits            ( g.userInt("cutBasedPhotonID-Fall17-94X-V1-loose") );
      ...
      const int mediumBits           ( g.userInt("cutBasedPhotonID-Fall17-94X-V1-medium") );
      ...
      const int tightBits            ( g.userInt("cutBasedPhotonID-Fall17-94X-V1-tight") );
      //bool passTightId( (tightBits & 0x3)== 0x3);  //require first two bits (h/e + sihih)
      const bool ismvawp80           ( g.photonID("mvaPhoID-RunIIFall17-v2-wp80"));
      const bool ismvawp90           ( g.photonID("mvaPhoID-RunIIFall17-v2-wp90"));
  ...
  for(auto jet = jets -> begin();  jet != jets -> end(); ++ jet)
    {
      //base kinematics
      if (jet -> pt() < 15.0 || fabs(jet -> eta()) > 4.7) 
	continue;
      ...
      if (!iEvent.isRealData())
        {
          ...
          ev_.j_flav[ev_.nj]       = jet -> partonFlavour();
          ev_.j_hadflav[ev_.nj]    = jet -> hadronFlavour();
          ev_.j_pid[ev_.nj]        = genParton ? genParton -> pdgId() : 0;
          for (int ig = 0; ig < ev_.ng; ig ++)
            {
              if (abs(ev_.g_id[ig]) == 11 || abs(ev_.g_id[ig]) == 13) 
		continue;
              if (deltaR( jet -> eta(), jet -> phi(), ev_.g_eta[ig], ev_.g_phi[ig]) > 0.4) 
		continue;
              genj_pt         = ev_.g_pt[ig];
              ev_.j_g[ev_.nj] = ig;
              ev_.g_xbp[ig]   = genParton   ? ev_.g_xb[ig] * genj_pt/genParton -> pt() : 0.0;
              break;
            }	 
          
          //jet energy resolution
          JME::JetParameters jerParams = {{JME::Binning::JetPt, jet -> pt()}, 
                                          {JME::Binning::JetEta, jet -> eta()},
                                          {JME::Binning::Rho, rho}};
          const float r = jerResolution.getResolution(jerParams);
          ...
          for(int ind = 0; ind < 3; ind ++) 
	    {
	      ...
	      if (genj_pt <= 0.0)
		{
		  const float sigma = r * std::sqrt(std::max(float( pow(jerSF[ind], 2)- 1.0), float(0.0)));
		  jerSF[ind] = std::max(float(1.0 + gRandom -> Gaus(0.0, sigma)), float(0.0));
		}
	      else 
		{           
		  const float dPt = jet -> pt() - genj_pt;
		  jerSF[ind] = std::max(float(1.0 + (jerSF[ind] - 1.0) * dPt / jet -> pt()), float(0.0));
		}
      ...
      const float NumChargedParticles = jet -> chargedMultiplicity();
      const float NumNeutralParticles = jet -> neutralMultiplicity();
      const float NumConst            = NumChargedParticles + NumNeutralParticles;
	...
	{ 
      ...
      //save jet
      ev_.j_area[ev_.nj]    = jet -> jetArea();
      ev_.j_jerUp[ev_.nj]   = jerSF[1];
      ev_.j_jerDn[ev_.nj]   = jerSF[2];
      for (size_t iunc = 0; iunc < jecCorrectionUncs_.size(); iunc++)
	{
	  jecCorrectionUncs_[iunc] -> setJetPt(jet -> pt());
	  jecCorrectionUncs_[iunc] -> setJetEta(jet -> eta());
      ...
      ev_.j_rawsf[ev_.nj]   = jet -> correctedJet("Uncorrected").pt()/jet -> pt();
      ev_.j_pt[ev_.nj]      = corrP4.pt();
      ev_.j_mass[ev_.nj]    = corrP4.mass();
      ev_.j_eta[ev_.nj]     = corrP4.eta();
      ev_.j_phi[ev_.nj]     = corrP4.phi();
      ev_.j_qg[ev_.nj]      = 0;//->userFloat("QGTagger:qgLikelihood");
      ev_.j_pumva[ev_.nj]   = jet -> userFloat("pileupJetId:fullDiscriminant");
      ev_.j_id[ev_.nj]      = jet -> userInt("pileupJetId:fullId");
      ev_.j_csv[ev_.nj]     = jet -> bDiscriminator("pfCombinedInclusiveSecondaryVertexV2BJetTags");
      ...
      ev_.j_c2_02[ev_.nj]    = getC(2, 0.2, &(*jet), true, 0.9);
      ev_.j_c2_05[ev_.nj]    = getC(2, 0.5, &(*jet), true, 0.9);
      ev_.j_c2_10[ev_.nj]    = getC(2, 1.0, &(*jet), true, 0.9);
      ev_.j_c2_20[ev_.nj]    = getC(2, 2.0, &(*jet), true, 0.9);
      ev_.j_zg[ev_.nj]       = getZg(&(*jet), true, 0.9)[0];
      ev_.j_mult[ev_.nj]     = calcGA(0, 0, &(*jet), true, 0.9);
      ev_.j_gaptd[ev_.nj]    = calcGA(0, 2, &(*jet), true, 0.9);
      ev_.j_gawidth[ev_.nj]  = calcGA(1, 1, &(*jet), true, 0.9);
      ev_.j_gathrust[ev_.nj] = calcGA(2, 1, &(*jet), true, 0.9);
      //this function throws an exception in rare events
      try
	{
	  ev_.j_tau32[ev_.nj]    = getTau(3, 2, &(*jet), true, 0.9);
	  ev_.j_tau21[ev_.nj]    = getTau(2, 1, &(*jet), true, 0.9);
	}
      catch(...)
	{
  ...
  for(size_t ind = 0; ind <14; ind ++)
    {
      ev_.met_ptShifted[ind]  = mets -> at(0).shiftedPt(pat::MET::METUncertainty(ind));
      ev_.met_phiShifted[ind] = mets -> at(0).shiftedPhi(pat::MET::METUncertainty(ind));
    }
  ...
  for (unsigned int ind = 0; ind < h_metFilters -> size(); ind ++) 
    {	
      if( !(* h_metFilters)[ind].accept() ) 
	continue;
      for(size_t itrig = 0; itrig < metFiltersToUse_.size(); itrig++)
	{
	  if (metFilterNames[ind].find(metFiltersToUse_[itrig]) == string::npos) 
	    continue;
  ...
  for(auto pf = pfcands -> begin();  pf != pfcands -> end(); ++ pf)
    {
      ...
      if (pf -> eta() > -3.0) ieta = 1;
      if (pf -> eta() > -2.5) ieta = 2;
      if (pf -> eta() > -1.5) ieta = 3;
      if (pf -> eta() >  0.0) ieta = 4;
...
bool MiniAnalyzer_test::isMediumMuon2016ReReco(const reco::Muon & recoMu) 
{
  ...
  const bool isMedium = muon::isLooseMuon(recoMu) && 
...
void MiniAnalyzer_test::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup)
{
  ...
  // printf("analyze probe B\n");
  
  //save event if at least one object at gen or reco level
  if (applyFilt_)
    if ((ngleptons_ == 0 && ngphotons_ == 0 && nrecleptons_ == 0 && nrecphotons_ == 0) || !saveTree_) 
      return;  
...
// 	{
// 	  const unsigned int nwgts(gen_header -> weightNames().size());
...
// 	    {
// 	      histContainer_["genlumiwgts"] -> GetXaxis() -> SetBinLabel(ind, it.c_str());
// 	      ind++;
// 	    }
// 	}
//       else
// 	{
// 	  histContainer_["genlumiwgts"] = fs -> make<TH1F>("genlumiwgts", ";Weight name;", 1, 0, 1);
				     ...
				     bool charged_only) 
{
  ...
  float deadcone_nh(0.0), deadcone_ch(0.0), deadcone_ph(0.0), deadcone_pu(0.0);
  if(ptcl -> isElectron()) 
    {
  ...
  float iso_nh(0.0), iso_ch(0.0), iso_ph(0.0), iso_pu(0.0);
  float ptThresh(0.5);
  if (ptcl -> isElectron()) 
    ptThresh = 0;
