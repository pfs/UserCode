...
#	python scripts/plotter.py ${commonOpts} -j ${json} --only HighMJJMM,LowMJJMM ${kFactors}
#       python scripts/plotter.py ${commonOpts} -j ${json} --only MM_evcount ${kFactors} --saveTeX -o evcout_plotter.root
#	python scripts/plotter.py ${commonOpts} -j ${syst_json} ${kFactors} --only HighMJJ,LowMJJ --silent -o syst_plotter.root
