...
{
  return TMath::Sqrt(2*a.Pt()*b.Pt()*(1.0 - TMath::Cos(a.DeltaPhi(b))));
}
...
//
FactorizedJetCorrector * getFactorizedJetEnergyCorrector_old(TString baseDir, bool isMC)
{
  ...
  for(size_t ind = 0; ind < jetCorFiles.size(); ind ++) 
    corSteps.push_back(JetCorrectorParameters(jetCorFiles[ind]));
...
std::vector<float> getJetResolutionScales_old(float pt, float eta, float genjpt)
{
  ...
  res[0] = TMath::Max((Float_t)0.0, (Float_t)(genjpt + (ptSF)*(pt - genjpt)))/pt;
  res[1] = TMath::Max((Float_t)0.0, (Float_t)(genjpt + (ptSF - ptSF_err)*(pt - genjpt)))/pt;
  res[2] = TMath::Max((Float_t)0.0, (Float_t)(genjpt + (ptSF + ptSF_err)*(pt - genjpt)))/pt;
...
//       if (nsyst_ > 0) 
// 	{
// 	  all2dPlots_[title + "_syst"] = new TH2F(title + "_syst", 
// 						  hist -> GetTitle(), 
// 						  hist -> GetNbinsX(), 
...
//       return;
//     }
//   if (allPlots_[title] -> InheritsFrom("TH2")) 
//     return;
...
//       all2dPlots_[title + "_syst"] -> Fill(value, 0.0, weights[0]);
//       for (unsigned int ind = 1; ind < weights.size(); ++ ind) 
// 	{
// 	  all2dPlots_[title + "_syst"] -> Fill(value, ind, weights[0] * weights[ind]);
// 	}
//     }
