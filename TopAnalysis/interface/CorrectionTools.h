...
TString assignRunPeriod(const std::vector<RunPeriod_t> & runPeriods, TRandom * rand = nullptr);
...
void smearJetEnergy_old(TLorentzVector & jp4, float genJet_pt, std::string option = "central");
...
void updateBTagDecisions_old(MiniEvent_t & ev, 
			     ...
			     std::string optionbc = "central", 
			     std::string optionlight = "central");
...
double computeSemilepBRWeight_old(MiniEvent_t & ev, std::map<int, double> corr, int pid = 0, bool useabs = true);
